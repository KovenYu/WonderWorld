...
class MarigoldPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if match_input_res:
            ...
            pred_img = pred_img.resize(input_size, Image.NEAREST)
    ...
    @torch.no_grad()
    def single_infer(
        self, rgb_in: torch.Tensor, num_inference_steps: int, show_pbar: bool, depth_conditioning: bool, target_depth: None, mask_align: None,
    ) -> torch.Tensor:
        """
        ...
        for i, t in iterable:
            if depth_conditioning and target_depth is not None and mask_align is not None and torch.any(mask_align):
                with torch.set_grad_enabled(depth_conditioning):
                    ...
                    print("-- {} noise {} grad {} noise/grad {}".format(i, (noise_pred**2).mean(), (dc_grad**2).mean(), (noise_pred**2).mean()/(dc_grad**2).mean()))                 
                    
                l2 = ((depth_orig - target_depth) ** 2 * mask_align)[mask_align > 0].mean()
                print("---- unet original {}: l2 {} l1 {}".format(i, l2, (torch.abs(depth_orig - target_depth) * mask_align)[mask_align > 0].mean()))

                # compute the previous noisy sample x_t -> x_t-1
                depth_latent = self.scheduler.step(noise_pred, t, depth_latent).prev_sample
                depth_latent.detach()

                depth_orig = self.decode_depth(depth_latent)
                depth_orig = torch.clip(depth_orig, -1.0, 1.0)
                depth_orig = (depth_orig + 1.0) / 2.0
                depth_orig = depth_orig / 200
                depth_orig = depth_orig + 0.001
                l2 = ((depth_orig - target_depth) ** 2 * mask_align)[mask_align > 0].mean()
                print("---- unet depth_conditioning {}: l2 {} l1 {}".format(i, l2, (torch.abs(depth_orig - target_depth) * mask_align)[mask_align > 0].mean()))
            else:
                unet_input = torch.cat(
                    [rgb_latent, depth_latent], dim=1
                )  # this order is important
